...
bool load_obj_file  (
	...
	points.reset((float*)malloc (3 * face_count * 3 * sizeof (float)));
	tex_coords.reset((float*)malloc (3 * face_count * 2 * sizeof (float)));
	normals.reset((float*)malloc (3 * face_count * 3 * sizeof (float)));
	printf (
	...
	const char *face_with_texture_coords = "f %i/%i/%i %i/%i/%i %i/%i/%i";
	const char *face_without_texture_coords = "f %i//%i %i//%i %i//%i";
	while (fgets (line, 1024, fp)) {
		// vertex
		if (line[0] == 'v') {
		...
		// faces
			...
			int vp[3], vt[3], vn[3];
			std::regex r("f[[:space:]]+[[:digit:]]+/[[:digit:]]+");
			bool has_texture_coords = std::regex_search(line, r);
			if (has_texture_coords) {
				sscanf (line, face_with_texture_coords,
						&vp[0], &vt[0], &vn[0], &vp[1], &vt[1], &vn[1], &vp[2], &vt[2], &vn[2]);
			} else {
				sscanf (line, face_without_texture_coords,
						&vp[0], &vn[0], &vp[1], &vn[1], &vp[2], &vn[2]);
				memset(vt, 0, sizeof (int) * 3);
			}
			...
			for (int i = 0; i < 3; i++) {
				...
				if ((vt[i] - 1 < 0 && has_texture_coords) || (vt[i] - 1 >= unsorted_vt_count)) {
				...
				points.get()[point_count * 3] = unsorted_vp_array[(vp[i] - 1) * 3];
				points.get()[point_count * 3 + 1] = unsorted_vp_array[(vp[i] - 1) * 3 + 1];
				points.get()[point_count * 3 + 2] = unsorted_vp_array[(vp[i] - 1) * 3 + 2];
				tex_coords.get()[point_count * 2] = unsorted_vt_array[(vt[i] - 1) * 2];
				tex_coords.get()[point_count * 2 + 1] = unsorted_vt_array[(vt[i] - 1) * 2 + 1];
				normals.get()[point_count * 3] = unsorted_vn_array[(vn[i] - 1) * 3];
				normals.get()[point_count * 3 + 1] = unsorted_vn_array[(vn[i] - 1) * 3 + 1];
				normals.get()[point_count * 3 + 2] = unsorted_vn_array[(vn[i] - 1) * 3 + 2];
				point_count++;
			}
		}
	}
